...
class TechBot:
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.api = MisskeyAPI(
            instance_url=config["instance_url"], access_token=config["access_token"]
        )
        self.running = False
        self.plugins = []
    ...
    def start(self):
        ...
        for plugin in self.plugins:
            try:
                ...
                logger.error(
                    f"プラグイン '{plugin.name}' の初期化中にエラーが発生しました: {e}"
                )
    ...
    def shutdown(self):
        ...
        for plugin in self.plugins:
            try:
                ...
                logger.error(
                    f"プラグイン '{plugin.name}' のシャットダウン中にエラーが発生しました: {e}"
                )
            ...
            if now.minute != last_minute:
                ...
                for plugin in self.plugins:
                    try:
                        ...
                        logger.error(
                            f"プラグイン '{plugin.name}' のタイマー処理中にエラーが発生しました: {e}"
                        )
        ...
        if msg_type == "mention":
            ...
            for plugin in self.plugins:
                try:
                    ...
                    logger.error(
                        f"プラグイン '{plugin.name}' のメンション処理中にエラーが発生しました: {e}"
                    )
            ...
            for plugin in self.plugins:
                try:
                    ...
                    logger.error(
                        f"プラグイン '{plugin.name}' のノート処理中にエラーが発生しました: {e}"
                    )
            ...
            for plugin in self.plugins:
                try:
                    ...
                    logger.error(
                        f"プラグイン '{plugin.name}' のメッセージ処理中にエラーが発生しました: {e}"
                    )
    ...
    def post_note(self, text, visibility="public", reply_id=None, files=None):
        ...
        try:
            ...
            result = self.api.post_note(
                text=text, visibility=visibility, reply_id=reply_id, files=files
            )
    ...
    def reply_to_note(self, note, text, visibility="public", files=None):
        ...
        return self.post_note(
            text=text, visibility=visibility, reply_id=note.get("id"), files=files
        )
