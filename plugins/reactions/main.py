...
class ReactionsPlugin(PluginBase):
    ...
    def __init__(self, bot, config=None):
        ...
        self.tech_keywords = self.config.get(
            "tech_keywords",
            [
                "プログラミング",
                "コード",
        ...
        self.tech_reactions = self.config.get(
            "tech_reactions", ["👍", "🚀", "💻", "✨", "🤖", "⚡", "🔥", "🎉"]
        )
        self.general_reactions = self.config.get(
            "general_reactions", ["👍", "😊", "✨", "🙌", "👀"]
        )
        self.reaction_probability = self.config.get("reaction_probability", 0.4)
        self.tech_reaction_probability = self.config.get(
            "tech_reaction_probability", 0.7
        )
    ...
    def on_note(self, note):
        ...
        # リアクションするかどうかの確率計算
        react_probability = (
            self.tech_reaction_probability
            if is_tech_related
            else self.reaction_probability
        )
