...
class PluginLoader:
        ...
        for item in os.listdir(plugin_dir):
            ...
            if os.path.isdir(item_path) and os.path.exists(os.path.join(item_path, 'main.py')):
                if enabled_plugins and item not in enabled_plugins:
                    logger.info(f"プラグイン '{item}' は有効化されていないためスキップします")
                    continue
    ...
    def load_plugin(self, plugin_name):
        ...
        try:
            ...
            if not plugin_class:
                logger.error(f"プラグイン '{plugin_name}' にプラグインクラスが見つかりません")
                return False
            ...
            logger.error(f"プラグイン '{plugin_name}' のロード中にエラーが発生しました: {e}")
            return False
    ...
    def unload_plugin(self, plugin_name):
        ...
        try:
            ...
            logger.error(f"プラグイン '{plugin_name}' のアンロード中にエラーが発生しました: {e}")
            return False
